...
let newHandler = function(type) {
  return {
    start(e) {
      ...
      // 防重处理
      let deltaT = Helper.getTimestamp() - start.T
      if (deltaT < 400) data.repeat = true
      ...
      /* 周期完成 */
      data.cycle = 3
      /* modifiers.canrepeat 单独添加的允许重复点击的修饰符-> 键盘场景可能会连续输入 */
      if (
        !type ||
        (!modifiers.canrepeat && data.repeat) ||
        Helper.getType(start, end) !== type
      ) { return false }
      /* 修饰符策略 */
      if (!modifierRules(this, e, modifiers)) {
        return false
...
vueTouch.install = function(Vue, options) {
  if (
    options &&
    options.afterEvent &&
    typeof options.afterEvent === 'function'
  ) { afterEvent = options.afterEvent }
  Vue.directive('touch', {
    bind(el, binding) {
      let type = binding.arg || 'tap' // 默认tap
      let modifiers = binding.modifiers
      let capture = !!modifiers.capture
      let method = binding.value || function() {}
      let key = ''
